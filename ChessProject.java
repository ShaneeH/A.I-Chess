...
//Iniatlize all the varibales we need
// We Use a LayeredPane to hold the ChessBoard and the Peices
//The ChessBoard is a JPanel, Peices are JLables
//We add a GridLayout to the ChessBoard Panel of 8x8 which gives us 64 squares wich we can do operations on
//We then make a JPanel called square and add these squares on top of our 8x8 grid we use loops to divide up these squares into 'Rows', 'Grey Squares and White Squares'
//We Manuually add the pictures of the chess peices to each square
...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
  ...
  Boolean white2Move;
  int moveCounter;
  // This is the instance of the AI Agent Class
  AIAgent agent;
  Boolean agentwins;
  Stack temporary;
  ...
  HashMap<String, Integer> chessMap = new HashMap<String, Integer>();
  private Object push;
  private Object push2;
  private Object push3;
  static int strat;

  public ChessProject() {
    Dimension boardSize = new Dimension(600, 600);
    // Hash Map lets us Key-Value the value to the chess peice
    chessMap.put("Pawn", PawnValue);
    chessMap.put("Knight", KnightValue);
    chessMap.put("Bishop", BishopValue);
    chessMap.put("Rook", RookValue);
    chessMap.put("Queen", QueenValue);
    chessMap.put("King", KingValue);
  ...
  private Stack getKingSquares(int x, int y, String piece) {
    ...
    if (!((tmpx2 < 0))) {
      ...
      if (checkSurroundingSquares(tmp3)) {
        validM = new Move(startingSquare, tmp3, setScore(((tmp3.getXC() * 75) + 20), ((tmp3.getYC() * 75) + 20), x, y));
  ...
  public int setScore(int x, int y, int lastX, int lastY) {
    ...
    try {
      ...
      // These are the rankings for the chess peices
      if (tmp1.contains(color + "Pawn")) {

        score = PawnValue;

      } else if (tmp1.contains(color + "Knight")) {

        score = KnightValue;

      } else if (tmp1.contains(color + "Bishop")) {

        score = BishopValue;

      } else if (tmp1.contains(color + "Rook")) {

        score = RookValue;

      } else if (tmp1.contains(color + "Queen")) {

        score = QueenValue;

      } else if (tmp1.contains(color + "King")) {
  ...
  private void makeAIMove() {
    ...
    // Creates a Stack with the position of all black pieces on the boards
    ...
    if (completeMoves.size() == 0) {
      ...
      // Feed the Data into the AIAgent Class
      Move selectedMove = agent.SelectedStrategy(strat, testing, black);
  ...
  public static void main(String[] args) {
    ...
    int StrategyPicked = JOptionPane.showOptionDialog(frame, "Lets play some Chess, choose your AI opponent",
        "Introduction to AI Continuous Assessment", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE,
        null, options, options[2]);
    strat = StrategyPicked;
    System.out.println("The selected variable is : " + StrategyPicked);
    frame.makeAIMove();
  }
}
